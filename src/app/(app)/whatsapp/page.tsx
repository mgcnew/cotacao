...
import {
  ...
  WhatsAppLiveUpdates,
} from "@/components/whatsapp/live-updates";
...
/**
 * Monta um link da caixa preservando o resto do recorte.
 *
 * Filtro, busca, conversa aberta e período são quatro coisas independentes, e
 * mexer numa não deveria apagar as outras. Antes os filtros montavam a URL do
 * zero — trocar de filtro no meio de uma busca perdia a busca e fechava a
 * conversa que estava sendo lida.
 */
function inboxHref(
  atual: { filtro: string; busca: string; conversa: string; periodo: number },
  mudanca: Partial<{
    filtro: string;
    busca: string;
    conversa: string;
  ...
  if (proximo.filtro && proximo.filtro !== "open")
    query.set("filtro", proximo.filtro);
  if (proximo.busca) query.set("busca", proximo.busca);
  if (proximo.conversa) query.set("conversa", proximo.conversa);
  if (proximo.periodo !== 30) query.set("periodo", String(proximo.periodo));
  const texto = query.toString();
  return texto ? `/whatsapp?${texto}` : "/whatsapp";
}
...
function messageTypeLabel(type: string) {
  return (
    ...
    )[type] ?? "Mensagem não suportada"
  );
}

function StatusIcon({ status }: { status: string }) {
  if (status === "failed")
    return <AlertCircle className="size-3 text-danger" aria-label="Falhou" />;
  if (["read", "played"].includes(status))
    return <CheckCheck className="size-3 text-sky-500" aria-label="Lida" />;
  if (status === "delivered")
...
export default async function WhatsAppPage({
    ...
    return (
      <div className="w-full">
        <PageHeader
          ...
          description="Conversas operacionais com fornecedores, ligadas às suas compras."
        />
        <div className="border-border bg-surface mx-auto mt-12 max-w-xl rounded-2xl border p-8 text-center shadow-sm">
          ...
          <h2 className="text-fg text-lg font-semibold">
            Ativar a caixa de compras
          </h2>
          <p className="text-fg-muted mx-auto mt-2 max-w-md text-sm">
            Conecte o número da empresa por QR Code nas Configurações. Nenhuma
            chave da Evolution será enviada ao navegador.
          </p>
          {errorMessage ? (
          ...
          {canManage ? (
            <Button asChild className="mt-6">
              <Link href="/configuracoes?aba=whatsapp">
                Configurar WhatsApp
              </Link>
            </Button>
          ) : (
            <p className="text-fg-subtle mt-5 text-xs">
              Um administrador precisa realizar a ativação inicial.
            </p>
          )}
        </div>
      </div>
  ...
  const [conversations, contacts, selected, metrics] = await Promise.all([
    ...
    selectedId
      ? getWhatsAppConversation(company.companyId, selectedId)
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected &&
          "md:max-lg:-mx-6 md:max-lg:-my-6 md:max-lg:h-[calc(100dvh-3.5rem)] md:max-lg:w-[calc(100%+3rem)]",
      )}
    >
      <div
        className={cn("max-md:px-4 max-md:pt-6", selected && "max-lg:hidden")}
      >
        <PageHeader
          title="WhatsApp Compras"
                ...
                <div className="flex items-center gap-2">
                  <form action={verifyWhatsAppConnectionAction}>
                    <Button type="submit" variant="outline" size="sm">
                      <RefreshCw aria-hidden /> Verificar
                    </Button>
                  </form>
                  <Button asChild variant="outline" size="sm">
                    <Link href="/configuracoes?aba=whatsapp">Configurar</Link>
                  </Button>
                </div>
              ) : null}
            </div>
      ...
      >
        <aside
          className={cn(
            "border-border min-h-0 min-w-0 max-w-full flex-col overflow-hidden border-r",
            selected ? "hidden lg:flex" : "flex",
          )}
        >
          <div className="border-border min-w-0 max-w-full space-y-3 border-b p-3">
            {/* A conversa aberta e o período viajam junto: buscar não deveria
                fechar o que está sendo lido nem reiniciar o recorte. */}
            <form className="relative min-w-0 max-w-full">
              ...
              <Input
                ...
                className={cn("pl-8", search && "pr-9")}
              />
              <input type="hidden" name="filtro" value={filter} />
              {selectedId ? (
                <input type="hidden" name="conversa" value={selectedId} />
              ) : null}
              {metricsDays !== 30 ? (
                <input type="hidden" name="periodo" value={metricsDays} />
              ) : null}
              {search ? (
                <Link
                  href={inboxHref(recorte, { busca: "" })}
                  aria-label="Limpar busca"
                  className="text-fg-subtle hover:text-fg absolute top-1.5 right-1.5 grid size-6 place-items-center rounded-md"
                >
            ...
            <div className="flex max-w-full gap-1 overflow-x-auto pb-0.5 text-xs">
              {[
                ...
                ["waiting_buyer", "Comprador"],
                ["promotions", "Promoções"],
              ].map(([value, label]) => (
                <Link
                  key={value}
                  href={inboxHref(recorte, { filtro: value })}
                  className={cn(
            ...
            {canSend && contacts.length ? (
              <form
                action={startWhatsAppConversationAction}
              ...
              >
                ...
                />
                <Tooltip content="Iniciar conversa" side="top">
                  {/* Era o `title` que dava nome a este botão: sem ele e sem
                      `aria-label`, um botão só de ícone fica anônimo. */}
                  <Button
                    type="submit"
                    size="icon"
                    className="shrink-0"
          ...
          <div className="min-h-0 min-w-0 max-w-full flex-1 overflow-y-auto overscroll-contain">
            {conversations.length ? (
              conversations.map((conversation) => (
                <Link
                  key={conversation.id}
                  href={inboxHref(recorte, { conversa: conversation.id })}
                  className={cn(
                    "border-border hover:bg-surface-muted flex w-full min-w-0 max-w-full gap-3 overflow-hidden border-b p-3 transition-colors",
                    selected?.id === conversation.id && "bg-primary/8",
                  )}
                >
                  <span className="bg-surface-muted text-fg-muted grid size-9 shrink-0 place-items-center rounded-full">
                    <UserRound className="size-4" aria-hidden />
                  </span>
                  <span className="min-w-0 flex-1 overflow-hidden">
                    <span className="flex min-w-0 items-baseline justify-between gap-2">
                      <strong className="text-fg min-w-0 flex-1 truncate text-sm font-medium">
                      ...
                      {conversation.last_message_at ? (
                        <time className="text-fg-subtle shrink-0 text-xs">
                    ...
                    <span className="mt-0.5 flex min-w-0 items-start gap-2">
                      <span className="text-fg-muted line-clamp-2 min-w-0 flex-1 wrap-anywhere text-xs">
                        {conversation.last_message_preview ??
                          "Conversa iniciada"}
                      </span>
                      {conversation.unread_count ? (
                        <span className="bg-primary-solid text-primary-solid-fg grid min-w-5 place-items-center rounded-full px-1.5 text-[11px] leading-5 font-semibold">
                          {conversation.unread_count}
                        </span>
                      ) : null}
                    </span>
                    <span className="text-fg-muted mt-1 block text-xs">
                      {conversation.awaiting_side === "supplier"
                        ? "Aguardando fornecedor"
                        : conversation.awaiting_side === "buyer"
        ...
        >
          {selected ? (
            <>
              <header className="border-border flex h-14 shrink-0 items-center gap-2 border-b px-2 sm:gap-3 sm:px-4">
                <Button asChild variant="ghost" size="sm" className="lg:hidden">
                  <Link href={inboxHref(recorte, { conversa: "" })}>
                    Voltar
                  </Link>
                </Button>
                <span className="min-w-0 flex-1">
                  ...
                  <span className="text-fg-subtle block truncate text-xs">
                    {selected.supplier_contacts?.name ??
                ...
                {selected.inbox_category === "promotion" ? (
                ...
                {selected.unread_count ? (
                  <Badge variant="secondary">
                    {selected.unread_count} não lidas
                  </Badge>
                ) : null}
                {canSend ? (
              ...
              {/* A conversa sem fornecedor vinculado precisa da ação em qualquer
                  tela. O formulário vivia só no painel lateral, que é
                  `hidden lg:block` — no celular a tela pedia o vínculo e não
                  oferecia como fazê-lo. Aqui ele acompanha o aviso. */}
              {!selected.supplier_id && canSend ? (
                <div className="border-warning/30 bg-warning/5 shrink-0 border-b px-3 py-2.5 sm:px-4">
                  <p className="text-fg flex items-center gap-2 text-xs font-medium">
                    <AlertCircle
                      className="text-warning size-3.5 shrink-0"
                      aria-hidden
                    />
                    Número ainda não vinculado a um fornecedor
                  </p>
                  <p className="text-fg-muted mt-1 text-xs">
                    Vincule antes de usar preços ou combinados desta conversa em
                    uma compra.
                  </p>
                  <form
                    action={linkWhatsAppConversationAction}
                  ...
                  >
                    ...
                    />
                    <SearchableSelect
                      id="whatsapp-link-contact-inline"
                      name="contact_id"
                      required
                      ...
                      options={contacts.map((contact) => ({
                        ...
                        name: `${contact.suppliers?.name ?? "Fornecedor"} · ${contact.name}`,
                        description:
                          contact.whatsapp ?? contact.phone ?? undefined,
                      }))}
                      placeholder="Digite fornecedor ou contato…"
                      emptyMessage="Nenhum contato encontrado."
                    />
                ...
                >
                  {selected.supplier_id ? (
                    <Button
                      ...
                      className="shrink-0"
                    >
                      <Link href={`/fornecedores/${selected.supplier_id}`}>
                        Fornecedor
                      </Link>
                    </Button>
                  ) : null}
                  ...
                  {messages.length ? (
                    messages.map((message) => (
                      ...
                      >
                        <div
                          className={cn(
                            "max-w-[86%] rounded-2xl px-3 py-2 text-sm shadow-xs sm:max-w-[72%]",
                            message.direction === "outbound"
                              ? "bg-primary/14 text-fg rounded-br-sm"
                              : "bg-surface-elevated text-fg rounded-bl-sm",
                          )}
                        >
                          ...
                          {message.message_type === "image" &&
                          message.media_path ? (
                            <Tooltip content="Abrir imagem" side="top">
                              <a
                          ...
                          <footer className="text-fg-muted mt-1 flex items-center justify-end gap-1 text-xs">
                            <time>
                    ...
                    <p className="text-fg-muted my-auto text-center text-sm">
            ...
            <div className="grid h-full place-items-center p-8 text-center">
              <div>
                <MessageCircle className="text-fg-subtle mx-auto size-9" />
                <h2 className="text-fg mt-3 font-medium">
                  Selecione uma conversa
                </h2>
                <p className="text-fg-muted mt-1 max-w-xs text-sm">
                  O histórico e o contexto da compra aparecem juntos, sem
                  misturar assuntos.
                </p>
              </div>
            </div>
        ...
        <aside className="border-border hidden min-h-0 overflow-y-auto border-l p-4 lg:block">
          {selected ? (
            <div className="space-y-5">
              <section>
                <p className="text-fg-subtle text-xs font-semibold tracking-wider uppercase">
                  Fornecedor
                </p>
                <h3 className="text-fg mt-2 font-medium">
                  {selected.suppliers?.name ?? "Não identificado"}
                </h3>
                <p className="text-fg-muted mt-1 flex items-center gap-1.5 text-xs">
                  <Phone className="size-3" />
                  {selected.normalized_phone ?? selected.remote_jid}
                </p>
                {selected.supplier_id ? (
                  <Link
                    href={`/fornecedores/${selected.supplier_id}`}
                  ...
                  <p className="text-warning mt-2 text-xs">
                    Ainda não vinculado — use o aviso no topo da conversa.
                  </p>
                )}
              </section>
              <section className="border-border border-t pt-4">
                ...
                <p className="text-fg-muted mt-2 text-xs">
                  {selected.awaiting_side === "supplier"
                    ? "Aguardando resposta do fornecedor"
                    : selected.awaiting_side === "buyer"
                      ? "Aguardando ação do comprador"
                      : "Sem pendência definida"}
                </p>
                {canSend ? (
                  <div className="mt-3 grid gap-2">
                    {(
                      [
                        ["supplier", "Aguardar fornecedor"],
                        ["buyer", "Aguardar comprador"],
                      ] as const
                    ).map(([lado, rotulo]) =>
                      selected.awaiting_side === lado ? null : (
                        <form
                          key={lado}
                          action={setWhatsAppConversationStateAction}
                        >
                          <input
                            type="hidden"
                            name="conversation_id"
                    ...
                    {/* Sem isto, marcar por engano deixava a conversa pendente na lista para sempre — não havia caminho de volta. */}
                    {selected.awaiting_side ? (
                      <form action={setWhatsAppConversationStateAction}>
                        <input
                          type="hidden"
                          name="conversation_id"
                          value={selected.id}
              ...
              {/* Antes esta seção explicava, o tempo todo, um botão que fica no
                  cabeçalho a poucos centímetros. Agora só relata o estado, e
                  apenas quando ele é fato. */}
              {selected.inbox_category === "promotion" ? (
                <section className="border-border border-t pt-4">
                  <p className="text-fg-subtle text-xs font-semibold tracking-wider uppercase">
                    Caixa
                  ...
                  <p className="text-fg-muted mt-2 text-xs">
                    Em Promoções: as mensagens continuam guardadas, mas não
                    geram pendências na caixa principal.
                  </p>
                </section>
              ) : null}
              <section className="border-border border-t pt-4">
                ...
                {context.round ? (
                  <Link
                    href={`/compras/${context.round.id}`}
                    className="bg-surface-muted mt-2 block rounded-lg p-3 text-xs"
                  >
                    <strong className="text-fg block">
                      {context.round.title}
                    </strong>
                    <span className="text-fg-muted">
                      Cotação · {context.round.status}
                    </span>
                  </Link>
                ) : (
                  <p className="text-fg-muted mt-2 text-xs">
                ...
                {context.order ? (
                  <Link
                    href={`/pedidos/${context.order.id}`}
                    className="bg-surface-muted mt-2 block rounded-lg p-3 text-xs"
                  >
                    <strong className="text-fg block">Pedido</strong>
                    <span className="text-fg-muted">
            ...
            <p className="text-fg-muted text-sm">
              O contexto do fornecedor aparecerá aqui.
            </p>
          )}
        </aside>
      </div>
      {/* Os indicadores vêm DEPOIS da caixa, e não antes: isto é uma caixa de
          entrada, e a primeira tela pertence às conversas, que são a tarefa.
          Em cima, empurravam o trabalho para baixo da dobra a cada abertura. */}
        ...
        hrefForPeriod={
          Object.fromEntries(
            ([7, 30, 90] as const).map((period) => [
              period,
              inboxHref(recorte, { periodo: period }),
            ]),
