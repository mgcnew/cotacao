...
function inboxHref(
  ...
  mudanca: Partial<{ filtro: string; busca: string; conversa: string; periodo: number }>,
) {
  const proximo = { ...atual, ...mudanca };
  const query = new URLSearchParams();
  if (proximo.filtro && proximo.filtro !== "open") query.set("filtro", proximo.filtro);
...
export default async function WhatsAppPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const recorte = { filtro: filter, busca: search, conversa: selectedId, periodo: metricsDays };
    ...
    >
        ...
        <aside className={cn("border-border min-h-0 min-w-0 max-w-full flex-col overflow-hidden border-r", selected ? "hidden lg:flex" : "flex")}>
          <div className="border-border min-w-0 max-w-full space-y-3 border-b p-3">
            ...
            <form className="relative min-w-0 max-w-full">
              ...
              <Input name="busca" defaultValue={search} placeholder="Buscar conversa" aria-label="Buscar conversa" className={cn("pl-8", search && "pr-9")} />
              <input type="hidden" name="filtro" value={filter} />
              {selectedId ? <input type="hidden" name="conversa" value={selectedId} /> : null}
              {metricsDays !== 30 ? <input type="hidden" name="periodo" value={metricsDays} /> : null}
              {search ? <Link href={inboxHref(recorte, { busca: "" })} aria-label="Limpar busca" className="text-fg-subtle hover:text-fg absolute top-1.5 right-1.5 grid size-6 place-items-center rounded-md"><X className="size-3.5" aria-hidden /></Link> : null}
            </form>
              ...
              >
                ...
                <span className="min-w-0 flex-1 overflow-hidden">
                  <span className="flex min-w-0 items-baseline justify-between gap-2">
                    ...
                    {conversation.last_message_at ? <time className="text-fg-subtle shrink-0 text-xs">{dateFormatter.format(new Date(conversation.last_message_at))}</time> : null}
                  </span>
          ...
          {selected ? (
            <>
              <header className="border-border flex h-14 shrink-0 items-center gap-2 border-b px-2 sm:gap-3 sm:px-4">
                <Button asChild variant="ghost" size="sm" className="lg:hidden"><Link href={inboxHref(recorte, { conversa: "" })}>Voltar</Link></Button>
              ...
              {!selected.supplier_id && canSend ? (
                <div className="border-warning/30 bg-warning/5 shrink-0 border-b px-3 py-2.5 sm:px-4">
                  <p className="text-fg flex items-center gap-2 text-xs font-medium"><AlertCircle className="text-warning size-3.5 shrink-0" aria-hidden />Número ainda não vinculado a um fornecedor</p>
                  <p className="text-fg-muted mt-1 text-xs">Vincule antes de usar preços ou combinados desta conversa em uma compra.</p>
                  <form action={linkWhatsAppConversationAction} className="mt-2.5 flex flex-wrap items-center gap-2">
                    <input type="hidden" name="conversation_id" value={selected.id} />
                    <SearchableSelect id="whatsapp-link-contact-inline" name="contact_id" required className="min-w-0 flex-1 sm:max-w-sm" options={contacts.map((contact) => ({ id: contact.id, name: `${contact.suppliers?.name ?? "Fornecedor"} · ${contact.name}`, description: contact.whatsapp ?? contact.phone ?? undefined }))} placeholder="Digite fornecedor ou contato…" emptyMessage="Nenhum contato encontrado." />
                    <Button type="submit" size="sm" className="shrink-0">Vincular</Button>
                  </form>
                </div>
              ) : null}
                  ...
                  {messages.length ? messages.map((message) => (
                    <article key={message.id} className={cn("flex", message.direction === "outbound" ? "justify-end" : "justify-start")}>
                      <div className={cn("max-w-[86%] rounded-2xl px-3 py-2 text-sm shadow-xs sm:max-w-[72%]", message.direction === "outbound" ? "bg-primary/14 text-fg rounded-br-sm" : "bg-surface-elevated text-fg rounded-bl-sm")}>
                        ...
                        {message.error_message ? <p className="text-danger mt-1 text-xs">{message.error_message}</p> : null}
                      </div>
                    </article>
        ...
        <aside className="border-border hidden min-h-0 overflow-y-auto border-l p-4 lg:block">
          {selected ? (
            <div className="space-y-5">
              <section><p className="text-fg-subtle text-xs font-semibold tracking-wider uppercase">Fornecedor</p><h3 className="text-fg mt-2 font-medium">{selected.suppliers?.name ?? "Não identificado"}</h3><p className="text-fg-muted mt-1 flex items-center gap-1.5 text-xs"><Phone className="size-3" />{selected.normalized_phone ?? selected.remote_jid}</p>{selected.supplier_id ? <Link href={`/fornecedores/${selected.supplier_id}`} className="text-primary mt-2 inline-block text-xs hover:underline">Abrir cadastro</Link> : <p className="text-warning mt-2 text-xs">Ainda não vinculado — use o aviso no topo da conversa.</p>}</section>
              <section className="border-border border-t pt-4"><p className="text-fg-subtle text-xs font-semibold tracking-wider uppercase">Andamento</p><p className="text-fg-muted mt-2 text-xs">{selected.awaiting_side === "supplier" ? "Aguardando resposta do fornecedor" : selected.awaiting_side === "buyer" ? "Aguardando ação do comprador" : "Sem pendência definida"}</p>{canSend ? <div className="mt-3 grid gap-2">{([["supplier", "Aguardar fornecedor"], ["buyer", "Aguardar comprador"]] as const).map(([lado, rotulo]) => selected.awaiting_side === lado ? null : <form key={lado} action={setWhatsAppConversationStateAction}><input type="hidden" name="conversation_id" value={selected.id} /><input type="hidden" name="awaiting_side" value={lado} /><Button type="submit" variant="outline" size="sm" className="w-full">{rotulo}</Button></form>)}{/* Sem isto, marcar por engano deixava a conversa pendente na lista para sempre — não havia caminho de volta. */}{selected.awaiting_side ? <form action={setWhatsAppConversationStateAction}><input type="hidden" name="conversation_id" value={selected.id} /><input type="hidden" name="awaiting_side" value="none" /><Button type="submit" variant="ghost" size="sm" className="text-fg-muted w-full">Limpar pendência</Button></form> : null}</div> : null}</section>
              ...
              {selected.inbox_category === "promotion" ? <section className="border-border border-t pt-4"><p className="text-fg-subtle text-xs font-semibold tracking-wider uppercase">Caixa</p><p className="text-fg-muted mt-2 text-xs">Em Promoções: as mensagens continuam guardadas, mas não geram pendências na caixa principal.</p></section> : null}
      ...
      <WhatsAppMetricsPanel
        ...
        hrefForPeriod={Object.fromEntries(
          ([7, 30, 90] as const).map((period) => [period, inboxHref(recorte, { periodo: period })]),
