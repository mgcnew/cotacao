...
export function renderSimpleRoundReportHtml(report: RoundReport) {
  ...
  const realizationHtml = realization
    ? realization.summary.postedReceiptCount > 0
      ? `<section class="realized${realization.summary.realizedSavings < 0 ? " loss" : ""}">
          <span>${realization.summary.realizedSavings >= 0 ? "Economia realizada após conferência" : "Acréscimo realizado após conferência"}</span>
          <strong>${escapeHtml(MONEY.format(Math.abs(realization.summary.realizedSavings)))}</strong>
          <p>Resultado sobre as quantidades já recebidas. Não deve ser somado à previsão da conclusão.</p>
          <div class="realized-details"><small>Negociada no recebido <b>${escapeHtml(MONEY.format(realization.summary.negotiatedSavingsOnReceived))}</b></small><small>Divergência nota x pedido <b>${escapeHtml(MONEY.format(realization.summary.divergenceImpact))}</b></small></div>
          <p class="coverage">${realization.summary.fullyReceivedItemCount} recebidos · ${realization.summary.partiallyReceivedItemCount} parciais · ${realization.summary.pendingItemCount} pendentes</p>
        </section>`
      : `<p class="notice"><strong>Resultado após conferência:</strong> ainda não há recebimento confirmado. A economia acima continua sendo a posição conhecida na conclusão.</p>`
    : "";
...
.realized{background:#f7f9fb;border:1px solid #b9c4cf;border-radius:14px;margin-top:12px;padding:17px;text-align:center;break-inside:avoid}.realized>span{color:#137a42;font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase}.realized>strong{color:#137a42;display:block;font-size:30px;margin-top:5px}.realized.loss>span,.realized.loss>strong{color:#b4232d}.realized>p{color:#53606b;font-size:12px;margin-top:6px}.realized-details{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:11px}.realized-details small{background:#fff;border:1px solid #dfe3e8;border-radius:8px;color:#69737d;padding:8px}.realized-details b{color:#17202a;display:block;font-size:14px;margin-top:3px}@media(max-width:620px){.realized-details{grid-template-columns:1fr}}
...
<p class="footer">Posição da conclusão gerada em ${escapeHtml(DATE.format(new Date(report.generatedAt)))}.${realization ? ` Recebimentos consultados em ${escapeHtml(DATE.format(new Date(realization.calculatedAt)))}.` : ""} Para consultar todos os preços e cálculos, abra o relatório completo.</p>
</main></body></html>`;
}
