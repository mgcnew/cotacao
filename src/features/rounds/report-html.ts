...
@media print{body{padding:0}.metrics{grid-template-columns:repeat(5,1fr)}.group{break-inside:auto}.item{break-inside:avoid}thead{display:table-header-group}}
...
<p class="footer">Gerado em ${escapeHtml(DATE_TIME.format(new Date(report.generatedAt)))}. A escolha de embalagens é uma métrica separada e exclusiva de produtos com essa finalidade. A economia realizada será apurada no recebimento.</p>
</main></body></html>`;
}
